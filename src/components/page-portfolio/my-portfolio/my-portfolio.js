...
export const ViewModel = DefineMap.extend({
  ...
  // To cancel an issuance we strip out issuance_tx_id and send as Empty EQB to a new EQB address. // DAVID
...
const sendIssuance = (portfolio, formData) => {
  ...
  // todo: for now just send the empty EQB change back to where we got it: // DAVID
  const changeAddrEmptyEqb = txoutsFee[0].address // DAVID

  const options = {
    fee: formData.transactionFee,
    changeAddr,
    changeAddrEmptyEqb,// DAVID
    amountEqb,
    type: 'TRANSFER',
    currencyType,
  ...
  return tx.save().then(() => {
    portfolio.markAsUsed(changeAddrEmptyEqb, 'EQB', true)// DAVID
  })
}
